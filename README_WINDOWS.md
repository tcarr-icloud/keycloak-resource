...
While developing this service, there are several dependencies. Keycloak is required, a Postgresql database, Docker, and
a few misc resources. This file contains most of these items and how they are being used during the development of this
application.

### Quick Start

1. Dockerfile Copy and paste into your system.
2. Start docker compose `docker compose up` (Keycloak is expected to fail starting because no db yet)
...
7. Login to keycloak and create realm by importing realm export
8. Create user in development realm, *api.user@development.com* with password as *password*
9. Assign the "Client Roles/realm-admin" to user.
10. Login to keycloak security console with user and verify realm stuff is available, you have permission.
11. Open project in Intellij.
12. Edit Run/Debug configurations, spring-boot template. Paste *intellij-run-debug-envs* string into the enviroment
    variables configuration template.
13. Run/debug main class *KeycloakResourceApplication*.
14. Verify service starts and connects to datasource.
15. Copy the *http-client.env.json* to file in project root.
15. Open *keycloak-resource.http* file and first 3 execute requests.
16. The previous steps are unverified as of writing this. **Good luck**.
